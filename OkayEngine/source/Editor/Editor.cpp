...
namespace Okay
{
	...
	void Editor::Update()
	{
		...
		if (ImGui::Begin("Other.."))
		{
			...
			ImGui::Text("FPS: %f, DT: %f", curFps, 1.f / curFps);
			ImGui::InputInt("FPS decrease", &loops);
	...
	void decreaseFps(int loops)
	{
		...
		{
			result /= std::sqrt(heavyFunction(result * (double)loops));
			result /= std::sqrt(heavyFunction(result * (double)loops));
			result /= std::sqrt(heavyFunction(result * (double)loops));
			result /= std::sqrt(heavyFunction(result * (double)loops));
			result /= std::sqrt(heavyFunction(result * (double)loops));
