...
bool Importer::Load(std::string_view filePath, Okay::Mesh::VertexData& outData, std::string* texPaths)
{
	...
	memcpy(outData.positions.data(), pMesh->mVertices, sizeof(Okay::Float3) * pMesh->mNumVertices);

	// Vertex UV
	outData.uvs.resize(pMesh->mNumVertices);
	memcpy(outData.uvs.data(), pMesh->mTextureCoords[0], sizeof(Okay::Float2) * pMesh->mNumVertices);
	
	// Vertex Normal
	outData.normals.resize(pMesh->mNumVertices);
	memcpy(outData.normals.data(), pMesh->mNormals, sizeof(Okay::Float3) * pMesh->mNumVertices);
