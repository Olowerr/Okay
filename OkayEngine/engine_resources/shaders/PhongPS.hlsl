...
float4 main(TransformedVertex input) : SV_TARGET
{
	...
	for (i = 0; i < numPoint; i++)
	{
        ...
        specShading += specular * pow(max(dot(reflect(-posToLight, normal), posToCam), 0.f), shinyness) *
			float4(poLight.colour, 1.f) * poLight.intensity * attu;
		
        diffShading += baseColour * max(dot(normal, posToLight), 0.f) * float4(poLight.colour, 1.f) * poLight.intensity * attu;
    }
	...
	for (i = 0; i < numDir; i++)
	{
        ...
        specShading += specular * pow(max(dot(reflect(dirLight.direction, normal), posToCam), 0.f), shinyness) *
			float4(dirLight.colour, 1.f) * dirLight.intensity;

        diffShading += max(dot(normal, -dirLight.direction), 0.f) * float4(dirLight.colour, 1.f) * dirLight.intensity;
    }
